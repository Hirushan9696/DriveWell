...
import 'dart:math'; // Make sure dart:math is imported

class MaintenanceRemindersPage extends StatefulWidget {
  final int userId;

  const MaintenanceRemindersPage({super.key, required this.userId});
...
class _MaintenanceRemindersPageState extends State<MaintenanceRemindersPage> {
  ...
  void _loadReminders() {
    setState(() {
      // Fetch only maintenance-related reminders for the user
      _remindersFuture = DatabaseHelper.instance.getRemindersForUser(widget.userId)
          .then((reminders) => reminders.where((r) =>
              r.type == 'Oil Change' ||
              r.type == 'Coolant Check' ||
              r.type == 'Tire Pressure Check' ||
              r.type == 'Wiper Fluid Check'
          ).toList());
    });
  }
  ...
  Future<void> _addReminder() async {
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (context) => AddMaintenanceReminderSheet(userId: widget.userId),
    );
    if (result == true) {
    ...
    try {
      ...
      if (reminder.intervalType == 'Mileage') {
        ...
        newNextDueDate = DateFormat('yyyy-MM-dd').format(
          DateTime(lastDate.year, lastDate.month + (int.parse(reminder.intervalValue.toString())), lastDate.day),
        );
      }
  ...
  IconData _getReminderIcon(String type) {
    switch (type) {
      case 'Oil Change': return Icons.oil_barrel;
      case 'Coolant Check': return Icons.local_drink;
      case 'Tire Pressure Check': return Icons.tire_repair;
      case 'Wiper Fluid Check': return Icons.cleaning_services;
                ...
                return Center(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.alarm_off, size: 80, color: Colors.black38), // Darker icon color
                        const SizedBox(height: 20),
                        const Text(
                          'No maintenance reminders set yet!',
                          style: TextStyle(color: Colors.black, fontSize: 22, fontWeight: FontWeight.bold, fontFamily: 'Inter'),
                          textAlign: TextAlign.center,
                        ),
                        const SizedBox(height: 10),
                        const Text(
                          'Tap the "+" button to add your first maintenance reminder.',
                        ...
                        builder: (context, vehicleSnapshot) {
                          ...
                          return Card(
                            ...
                            shape: RoundedRectangleBorder(
                              ...
                              side: BorderSide(
                                color: isOverdue ? Colors.redAccent : Colors.black26, // Darker border if overdue
                                ...
                                children: [
                                  Row(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          Icon(_getReminderIcon(reminder.type), color: Colors.black, size: 28),
...
class AddMaintenanceReminderSheet extends StatefulWidget {
  final int userId;

  const AddMaintenanceReminderSheet({super.key, required this.userId});

  @override
  State<AddMaintenanceReminderSheet> createState() => _AddMaintenanceReminderSheetState();
}

class _AddMaintenanceReminderSheetState extends State<AddMaintenanceReminderSheet> with TickerProviderStateMixin {
  final _formKey = GlobalKey<FormState>();
  String? _selectedVehicleId;
  String _selectedReminderType = 'Oil Change';
  String _selectedIntervalType = 'Mileage';
  ...
  Future<void> _saveReminder() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        // Fetch the current vehicle's mileage to set as lastTriggeredMileage if applicable
        ...
        if (_selectedIntervalType == 'Mileage') {
          ...
          nextDueDate = DateFormat('yyyy-MM-dd').format(
            DateTime(lastDate.year, lastDate.month + (int.parse(_intervalValueController.text.trim())), lastDate.day),
          );
        }
        ...
        if (result > 0) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Reminder added successfully!', style: TextStyle(fontFamily: 'Inter'))),
            );
              ...
              children: <Widget>[
                ...
                DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: const Icon(Icons.directions_car, color: Colors.black), // Darker icon color
                  ...
                  style: const TextStyle(color: Colors.black, fontFamily: 'Inter'), // Dark text
                ...
                DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: const Icon(Icons.category, color: Colors.black), // Darker icon color
                    fillColor: Colors.white.withOpacity(0.2),
                    filled: true,
                    border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(12.0),
                  ...
                  items: <String>[
                    ...
                    return DropdownMenuItem<String>(
                      value: value,
                      child: Text(value, style: const TextStyle(color: Colors.black, fontFamily: 'Inter')), // Dark text
                    );
                  }).toList(),
                  onChanged: (String? newValue) {
                    setState(() {
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintText: _selectedIntervalType == 'Mileage' ? 'e.g., 5000' : 'e.g., 3',
                ...
                _selectedIntervalType == 'Date'
                    ...
                    : TextFormField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration: InputDecoration(
                          labelText: 'Last Triggered Mileage (km)',
                          hintText: 'e.g., 50000',
                        ...
                        validator: (value) {
                          if (value == null || value.isEmpty) {
                            return 'Please enter last triggered mileage';
                          }
                          if (double.tryParse(value) == null || double.parse(value) < 0) {
