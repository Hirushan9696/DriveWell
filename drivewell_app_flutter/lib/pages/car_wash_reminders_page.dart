// lib/pages/car_wash_reminders_page.dart
...
class CarWashRemindersPage extends StatefulWidget {
  final int userId;

  const CarWashRemindersPage({super.key, required this.userId});

  @override
  State<CarWashRemindersPage> createState() => _CarWashRemindersPageState();
}

class _CarWashRemindersPageState extends State<CarWashRemindersPage> {
  ...
  void _loadReminders() {
    setState(() {
      // Fetch only 'Car Wash' reminders for the user
      _remindersFuture = DatabaseHelper.instance.getRemindersForUser(widget.userId)
          .then((reminders) => reminders.where((r) => r.type == 'Car Wash').toList());
    });
  }
  ...
  Future<void> _addReminder() async {
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (context) => AddCarWashReminderSheet(userId: widget.userId),
    );
    if (result == true) {
                ...
                return Center(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.car_crash, size: 80, color: Colors.black38), // Darker icon color
                        const SizedBox(height: 20),
                        const Text(
                          'No car wash reminders set yet!',
                          style: TextStyle(color: Colors.black, fontSize: 22, fontWeight: FontWeight.bold, fontFamily: 'Inter'),
                          textAlign: TextAlign.center,
                        ),
                        const SizedBox(height: 10),
                        const Text(
                          'Tap the "+" button to add your first car wash reminder.',
                                ...
                                children: [
                                  Row(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          Icon(Icons.car_crash, color: Colors.black, size: 28), // Car wash icon
...
class AddCarWashReminderSheet extends StatefulWidget {
  final int userId;

  const AddCarWashReminderSheet({super.key, required this.userId});

  @override
  State<AddCarWashReminderSheet> createState() => _AddCarWashReminderSheetState();
}

class _AddCarWashReminderSheetState extends State<AddCarWashReminderSheet> {
  final _formKey = GlobalKey<FormState>();
  String? _selectedVehicleId;
  String _selectedIntervalType = 'Mileage'; // Default for car wash
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        final Reminder newReminder = Reminder(
          ...
          type: 'Car Wash', // Fixed type for this sheet
        ...
        if (result > 0) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Car Wash Reminder added successfully!', style: TextStyle(fontFamily: 'Inter'))),
            );
            Navigator.pop(context, true);
          }
        } else {
          setState(() {
            _errorMessage = 'Failed to add car wash reminder.';
          });
        }
      } catch (e) {
        setState(() {
        ...
        print('Error saving car wash reminder: $e');
      } finally {
        setState(() {
          _isLoading = false;
        });
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    labelText: _selectedIntervalType == 'Mileage' ? 'Interval Value (km)' : 'Interval Value (days)', // Changed to days for car wash
                    hintText: _selectedIntervalType == 'Mileage' ? 'e.g., 500' : 'e.g., 7',
                ...
                _selectedIntervalType == 'Date'
                    ...
                    : TextFormField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration: InputDecoration(
                          labelText: 'Last Washed Mileage (km)',
                          hintText: 'e.g., 50000',
                        ...
                        validator: (value) {
                          if (value == null || value.isEmpty) {
                            return 'Please enter last washed mileage';
                          }
                          if (double.tryParse(value) == null || double.parse(value) < 0) {
