// lib/pages/document_expiry_reminders_page.dart
...
class DocumentExpiryRemindersPage extends StatefulWidget {
  final int userId;

  const DocumentExpiryRemindersPage({super.key, required this.userId});

  @override
  State<DocumentExpiryRemindersPage> createState() => _DocumentExpiryRemindersPageState();
}

class _DocumentExpiryRemindersPageState extends State<DocumentExpiryRemindersPage> {
  ...
  void _loadReminders() {
    setState(() {
      // Fetch only 'Document Expiry' reminders for the user
      _remindersFuture = DatabaseHelper.instance.getRemindersForUser(widget.userId)
          .then((reminders) => reminders.where((r) => r.type == 'Document Expiry').toList());
    });
  }
  ...
  Future<void> _addReminder() async {
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (context) => AddDocumentExpiryReminderSheet(userId: widget.userId),
    );
    if (result == true) {
  ...
  Future<void> _markReminderAsComplete(Reminder reminder) async {
    ...
    try {
      ...
      double? currentMileage; // Not relevant for document expiry, but kept for consistency

      // Calculate new nextDueDate/nextDueMileage
      String? newNextDueDate;
      double? newNextDueMileage;

      // For document expiry, we assume it's always date-based and the intervalValue
      // represents the duration (e.g., 1 year, 6 months) from the *current date*
      // or the last triggered date.
      if (reminder.intervalType == 'Date') {
        final lastDate = DateTime.parse(currentDate); // Use current date as last triggered
        // Assuming intervalValue for Document Expiry is in months
      ...
      } else { // Handle mileage case if it somehow gets set for document expiry
      ...
      final updatedReminder = reminder.copyWith(
        ...
        lastTriggeredMileage: currentMileage, // Will be null for date-based
        nextDueDate: newNextDueDate,
        nextDueMileage: newNextDueMileage, // Will be null for date-based
      );
  ...
  Widget _buildReminderDetails(Reminder reminder) {
    ...
    if (reminder.intervalType == 'Mileage') {
      ...
      details += 'Interval: ${reminder.intervalValue.toInt()} months'; // Assuming months for document expiry
                ...
                return Center(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.description, size: 80, color: Colors.black38), // Darker icon color
                        const SizedBox(height: 20),
                        const Text(
                          'No document expiry reminders set yet!',
                          style: TextStyle(color: Colors.black, fontSize: 22, fontWeight: FontWeight.bold, fontFamily: 'Inter'),
                          textAlign: TextAlign.center,
                        ),
                        const SizedBox(height: 10),
                        const Text(
                          'Tap the "+" button to add your first document expiry reminder.',
                                ...
                                children: [
                                  Row(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          Icon(Icons.description, color: Colors.black, size: 28), // Document expiry icon
...
// AddDocumentExpiryReminderSheet (as a separate widget or internal class)
class AddDocumentExpiryReminderSheet extends StatefulWidget {
  final int userId;

  const AddDocumentExpiryReminderSheet({super.key, required this.userId});

  @override
  State<AddDocumentExpiryReminderSheet> createState() => _AddDocumentExpiryReminderSheetState();
}

class _AddDocumentExpiryReminderSheetState extends State<AddDocumentExpiryReminderSheet> {
  final _formKey = GlobalKey<FormState>();
  String? _selectedVehicleId;
  String _selectedIntervalType = 'Date'; // Default for document expiry
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        final Reminder newReminder = Reminder(
          ...
          type: 'Document Expiry', // Fixed type for this sheet
          intervalType: _selectedIntervalType, // Will be 'Date'
          ...
          lastTriggeredMileage: null, // Not applicable for document expiry
          nextDueDate: nextDueDate,
          nextDueMileage: null, // Not applicable for document expiry
        ...
        if (result > 0) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Document Expiry Reminder added successfully!', style: TextStyle(fontFamily: 'Inter'))),
            );
            Navigator.pop(context, true);
          }
        } else {
          setState(() {
            _errorMessage = 'Failed to add document expiry reminder.';
          });
        }
      } catch (e) {
        setState(() {
        ...
        print('Error saving document expiry reminder: $e');
      } finally {
        setState(() {
          _isLoading = false;
        });
              ...
              children: <Widget>[
                ...
                DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: const Icon(Icons.directions_car, color: Colors.black), // Darker icon
                ...
                // Only Date is available for Document Expiry, no Mileage option
                DropdownButtonFormField<String>(
                  value: _selectedIntervalType,
                  decoration: InputDecoration(
                    labelText: 'Interval Type',
                    prefixIcon: const Icon(Icons.timer, color: Colors.black), // Darker icon
                    fillColor: Colors.white.withOpacity(0.2),
                    filled: true,
                    border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(12.0),
                  ...
                  dropdownColor: Colors.blueGrey.shade800,
                  style: const TextStyle(color: Colors.black, fontFamily: 'Inter'),
                  items: <String>['Date'] // Only 'Date' for Document Expiry
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    labelText: 'Interval Value (months)', // Always months for document expiry
                    hintText: 'e.g., 12 (for 1 year)',
                    prefixIcon: const Icon(Icons.calendar_month, color: Colors.black), // Dark icon color
                    fillColor: Colors.white.withOpacity(0.2),
                    filled: true,
                    border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(12.0),
                ...
                TextFormField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'Last Issued/Renewed Date',
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintText: 'e.g., Insurance, Registration, License',
